...
const AdvancedBillingFeatures = () => {
  ...
  const loadAdvancedFeatures = async () => {
    try {
      ...
      // Load recurring billing schedules from Firestore
      try {
        ...
        const mapped = schedules.map(s => ({
          ...
          nextRun: s.nextRunDate || new Date(),
          isActive: s.isActive ?? false,
          studentCount: s.studentIds?.length || 0,
        }));
        const activeSchedules = mapped.filter(s => s.isActive);
        setRecurringBilling({
          schedules: mapped,
          nextRun: activeSchedules.length > 0 ? activeSchedules[0].nextRun : null,
          isEnabled: activeSchedules.length > 0,
        });
      } catch (scheduleError) {
        console.warn('Could not load billing schedules:', scheduleError);
        setRecurringBilling({ schedules: [], nextRun: null, isEnabled: false });
      }
    } catch (error) {
          ...
          <Grid item xs={12} md={6} key={schedule.id}>
            ...
            >
              <CardContent>
                ...
                >
                  Next Run: {schedule.nextRun ? new Date(schedule.nextRun).toLocaleDateString() : 'N/A'}
                </Typography>
