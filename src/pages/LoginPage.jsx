...
function LoginPage() {
  ...
  useEffect(() => {
    const checkAutoFill = () => {
      // If both email and password are filled (auto-fill or manual), require auth code
      if (email && password && !showAuthCode) {
        console.log('🔐 Email and password detected - requiring authorization code');
        setShowAuthCode(true);
        setError('Please enter your authorization code to complete login.');
      }
    };

    // Check immediately and after delay for auto-fill
    checkAutoFill();
    const timer = setTimeout(checkAutoFill, 500);
    return () => clearTimeout(timer);
  }, [email, password]);

  // Clear error when auth code is entered
  useEffect(() => {
    if (authorizationCode && error.includes('authorization code')) {
      setError('');
    }
  ...
  const handleLogin = async event => {
    ...
    try {
      ...
      // ALWAYS require authorization code when both email and password are present
      ...
      // Validate authorization code (basic validation - you can enhance this)
      if (authorizationCode.length < 3) {
      ...
      <Container component="main" maxWidth="xs">
        ...
        >
          ...
          <Box component="form" onSubmit={handleLogin}>
            ...
            {/* Authorization Code Field - Always Required */}
            <Box sx={{ mb: 2 }}>
              {(email && password) && (
                <Typography
                  variant="body2"
              ...
              {(showAuthCode || (email && password)) && (
                <TextField
                  fullWidth
                  name="authorizationCode"
                  label="Authorization Code (Required)"
                  required
                  type="text"
