...
class FirebaseService {
  ...
  async getPaymentRecord(paymentId) {
    ...
    }, `Get Payment Record ${paymentId}`);
  }

  async updatePaymentRecord(paymentId, paymentData) {
    return this.withRetry(async () => {
      const paymentRef = doc(db, 'payments', paymentId);
    ...
    }, `Update Payment Record ${paymentId}`);
  }

  async getPaymentsForInvoice(invoiceId) {
    return this.withRetry(async () => {
      const paymentsRef = collection(db, 'payments');
      const q = query(paymentsRef, where('invoiceId', '==', invoiceId), orderBy('createdAt', 'desc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    }, `Get Payments for Invoice ${invoiceId}`);
  }

  // ==================== Refund Records ====================

  async addRefundRecord(refundData) {
    return this.withRetry(async () => {
  ...
  async getRefundsForPayment(paymentId) {
    return this.withRetry(async () => {
      const refundsRef = collection(db, 'refunds');
      const q = query(refundsRef, where('paymentId', '==', paymentId), orderBy('createdAt', 'desc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    }, `Get Refunds for Payment ${paymentId}`);
  }

  async updateRefundRecord(refundId, refundData) {
    return this.withRetry(async () => {
  ...
  async getBillingSchedules() {
    return this.withRetry(async () => {
      const schedulesRef = collection(db, 'billingSchedules');
      ...
      return snapshot.docs.map(d => {
        ...
        return {
          ...
          nextRunDate: data.nextRunDate?.toDate ? data.nextRunDate.toDate() : data.nextRunDate ? new Date(data.nextRunDate) : null,
          lastRunDate: data.lastRunDate?.toDate ? data.lastRunDate.toDate() : data.lastRunDate ? new Date(data.lastRunDate) : null,
        };
      });
    }, 'Get Billing Schedules');
  }

  async createBillingSchedule(scheduleData) {
    return this.withRetry(async () => {
      const schedulesRef = collection(db, 'billingSchedules');
  ...
  async updateBillingSchedule(scheduleId, scheduleData) {
    return this.withRetry(async () => {
      const scheduleRef = doc(db, 'billingSchedules', scheduleId);
      await updateDoc(scheduleRef, {
        ...scheduleData,
        updatedAt: serverTimestamp(),
      });
    }, `Update Billing Schedule ${scheduleId}`);
  }

  async deleteBillingSchedule(scheduleId) {
    return this.withRetry(async () => {
      const scheduleRef = doc(db, 'billingSchedules', scheduleId);
      await deleteDoc(scheduleRef);
    }, `Delete Billing Schedule ${scheduleId}`);
  }
}
