...
class BillingService {
  ...
  // Process a refund as account credit for a student
  async processRefundAsCredit(studentId, refundData) {
    try {
      const credit = await this.addStudentCredit(studentId, {
        amount: refundData.amount,
        currency: refundData.currency || 'ZAR',
        source: 'refund',
        description: refundData.reason
          ? `Refund: ${refundData.reason}`
          : `Refund from invoice ${refundData.invoiceId || ''}`,
      });
      return credit;
    } catch (error) {
      console.error('Failed to process refund as credit:', error);
      throw error;
    }
  }
