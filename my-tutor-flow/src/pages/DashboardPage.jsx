...
import { isMobile as checkIsMobile } from '../config/mobileConfig';
import {
...
const DashboardPage = () => {
  ...
  // ULTRA-FAST data fetching with caching
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (cached && cacheTime && (now - parseInt(cacheTime)) < 300000) {
        ...
        setStats({ ...cachedData, loading: false });
        return;
      }
  ...
  // Show mobile dashboard on mobile devices or when mobile config is enabled
  if (isMobile || checkIsMobile()) {
